...
def main():
    ...
    config = Config()
    # build model
    model = NERModel(config, LASEREmbedderII(config.model_path))
    ...
    dev = parse_dataset(config.filename_dev, config.label_to_idx, config.word_to_idx) #TODO: change back to dev set
